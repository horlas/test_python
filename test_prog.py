...
import os
import unittest
from prog import text_lower, text_only_word, list_of_words, sort_word_by_frequency, list_sentences, count_sentences

class SomeTest(unittest.TestCase):

	def setUp(self):
		self.path = os.path.join(os.getcwd(), 'test_text.txt')
		self.path_sentences = os.path.join(os.getcwd(), 'long_text.txt')
	...
	def test_list_sentences(self):
		sentences = list_sentences(self.path_sentences)
		self.assertEqual(len(count_sentences(sentences)), 18)
